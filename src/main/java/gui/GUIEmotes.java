...
public class GUIEmotes extends JFrame {
    ...
    public void refreshEmotes() {
    	...
    	if (this.panel.getComponentCount() > 0) this.panel.removeAll();
    	SortedMap<Integer, TwitchFace> sortedFaceMap = new TreeMap<Integer, TwitchFace>(FaceManager.twitchFaceMap);
    	for (Map.Entry<Integer, TwitchFace> entry : sortedFaceMap.entrySet()) {
        	emote = new JLabel();
        	...
        	emoteName = handleRegex(entry.getValue().getRegex().replaceAll("\\?", "").replaceAll("\\\\", ""));
        	emote.setName(emoteName);
        	emote.setToolTipText(emoteName);
        	URL image;
        	try{
        	image = new File(entry.getValue().getFilePath()).toURI().toURL();
