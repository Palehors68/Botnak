...
public class GUIMain extends JFrame {
	...
	public static GUIEmotesPHB emotes = null;
	...
	public void exitButtonActionPerformed() {
		...
		for (String c: channelSet) {
			Channel ch = Settings.channelManager.getChannel(c);
			if (ch != null) ch.clear();
		}
		if (viewer != null) {
			viewer.close(false);
	...
	public void emoteButtonActionPerformed() {
		if (emotes == null) {
			emotes = new GUIEmotesPHB(userChat, this);
		}
    ...
    public void setWhisperModeToggle(){
    	Settings.botWhisperMode.getValue();
    }
	...
	private void whisperModeToggleStateChanged(){
		Settings.botWhisperMode.setValue(whisperModeToggle.isSelected());
	}
	...
	private void ffzEmotesToggleItemStateChanged(){
		GUIEmotesPHB.setFfzEmoteToggle(ffzEmotesToggle.isSelected());
		if (emotes != null)
			emotes.refreshEmotes();
	}
